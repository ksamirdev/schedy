---
title: "Update a task"
description: "PUT /tasks/{id} - replace a pending task's schedule and delivery settings while keeping its id."
---
...
Use this to reschedule, retarget, or re-arm a task without the caller having to track a new id the way delete-then-recreate would force.

Only `pending` tasks can be updated.
Anything else - `running`, `succeeded`, `failed`, `cancelled` - is a `409`.
See [Status & History](/concepts/status).
...
The [key](/concepts/idempotency) in particular is set once at creation and is not settable here - it names the task, not what the task does.
...
Identical to [create](/api/create).
...
| `retry_mode`     | string | Optional retry timing: `fixed` (default) or `exponential`. Omitted = `fixed`. See [Retries](/concepts/retries). |

Unlike create, `PUT` never deduplicates and ignores `Idempotency-Key` - the id in the path already says which task you mean.
...
The scheduler picks up work slightly ahead of time, so an update can land while a task is already queued for delivery.
That case is handled rather than raced: the scheduler re-reads the task immediately before firing, so a `200` response means the update is what will be delivered.
