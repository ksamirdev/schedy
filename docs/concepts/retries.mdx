...
The callback is fire-and-forget: it is never retried, and a failing callback never triggers a callback about itself. It is global (one URL for the whole server) and failure-only - success does not call back.
