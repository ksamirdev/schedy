...
A task can override the global URL with its own `on_failure_url` field, so different jobs can page different receivers.
The per-task URL wins when both are set; the payload and semantics are identical.
